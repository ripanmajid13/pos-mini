...
@push('script_inline')
    <script>
        ...
        UpdateForm = (me, text, url, roles = false) => {
        ...
        let dataTable = $("#dataTable").DataTable({
            ...
            columns: [
                ...
                @canany([$canRoles, $canEdit, $canDelete])
        ...
        $('body').on('submit', '#form-action', function(e) {
            ...
            if ($(this).find('.btn-submit').html() == 'TRANSFER TO USER') {
                Swal.fire({
                    icon: 'warning',
                    title: 'Apakah kamu yakin ?',
                    text: `Transfer guest ${$(this).find('#name').val()} to user.`,
                    ...
                    if (result.value) {
                        UpdateForm($(this), $(this).find('.btn-submit').html(), $(this).attr('action'), true);
                    }
                })
            } else {
                UpdateForm($(this), $(this).find('.btn-submit').html(), $(this).attr('action'));
            }
        });
        ...
        $('body').on('click', '.btn-roles', function(e) {
            ...
            loadingForm();
            $.post($(this).attr('href'), { "_token" : $('meta[name="csrf-token"]').attr('content') },  function(response) {
                $('.card-title').html(`Add Roles To Guest`);
                $('#body-form').html(response);
                $('.select2').select2({
                    placeholder: "Choose"
                });
